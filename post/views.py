...
class JobView(APIView):
    def post(self, request):
        job_post_serializer = JobPostSerializer(data = request.data, context = {'request': request})
        if job_post_serializer.is_valid():
            # job_type= int(request.data.get("job_type", None))
            # company_name = request.data.get("company_name", None)
            # skillsets = request.data.get('skillsets', None)z
            # skillsets = list(map(int, skillsets))
